...
from app.schemas.responses import ValidationData
...
class ValidationAgent(BaseAgent):
    def __init__(self):
        super().__init__(
            ...
            response_schema=ValidationData,
            prompt_file="validation.txt"
        )

    def run(self, startup_info: Dict[str, Any], discovery_summary: str) -> Dict[str, Any]:
        variables = {
        ...
        summary = f"Validation Quotient: {data.validation_score}%. Feasibility: {data.feasibility}%. Reasoning: {data.reasoning[:60]}."
